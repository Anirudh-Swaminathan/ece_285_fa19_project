...
class Experiment(object):
    ...
    def __init__(self, net, train_set, val_set, optimizer, stats_manager, collate_func=None,
                 output_dir=None, batch_size=128, perform_validation_during_training=False):
        ...
        train_loader = td.DataLoader(train_set, batch_size=batch_size, shuffle=True,
                                     ...
                                     worker_init_fn=torch.manual_seed(7))
        val_loader = td.DataLoader(val_set, batch_size=batch_size, shuffle=False, collate_fn=collate_func,
    ...
    def epoch(self):
        ...
        epoch_number = math.floor(len(self.history)/len(self.train_loader))
        batch_number = len(self.history)%len(self.train_loader)
        return epoch_number,batch_number	
    ...
    def run(self, num_epochs, plot=None):
        ...
        start_epoch, start_batch = self.epoch
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for batch in range(start_batch, len(self.train_loader)):
                ...
                x, d, cap_len = x.to(self.net.device), d.to(self.net.device), cap_len.to(self.net.device)
                target = pack_padded_sequence(d, cap_len, batch_first=True)[0]
                self.optimizer.zero_grad()
                y = self.net.forward(x,d,cap_len)
                #print("Printing the dimensions of x, y, d and target")
                #print(x.size())
                #print(y.size())
                #print(d.size())
                #print(target.size())
                #print("Printed x, y and d, target size in that order")
                loss = self.net.criterion(y, target)
                with open(self.output_dir+'losses.txt','a') as t_file:
                    print('Epoch:{} Batch:{} Loss:{}'.format(epoch, batch, loss), file=t_file)
    ...
    def evaluate(self):
        ...
        self.net.eval()
        with torch.no_grad():
            for x, d, cap_len in self.val_loader:
                x, d, cap_len = x.to(self.net.device), d.to(self.net.device), cap_len.to(self.net.device)
                target = pack_padded_sequence(d,cap_len,batch_first=True)[0]
                y = self.net.forward(x, d, cap_len)
                loss = self.net.criterion(y, target)
